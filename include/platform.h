...
//! \brief the nim size of a malloc-able chunk of a heap which we will put into
//! the fake heap.
...
static inline bool platform_new_heap_creation(
    ...
    // NOTE use if not trusting the fake heap. uncomment out the next 2 lines
    ...
    sdram_block *list_of_available_blocks =
        sark_alloc(n_mallocs * sizeof(sdram_block), 1);
